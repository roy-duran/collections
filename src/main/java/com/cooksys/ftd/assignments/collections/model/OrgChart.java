...
public class OrgChart {
	...
	public Set<Employee> getDirectSubordinates(Manager manager) {
		...
		for (Employee e : getAllEmployees()) {
			if (hasEmployee(manager) && e.hasManager()) {
	...
	public Map<Manager, Set<Employee>> getFullHierarchy() {
		Map<Manager, Set<Employee>> hierarchyMap = new HashMap<>();
		Set<Manager> managerSet = new HashSet<>(getAllManagers());
		for (Manager m : managerSet) {
			hierarchyMap.put(m, getDirectSubordinates(m));
		}
		return hierarchyMap;
	}
